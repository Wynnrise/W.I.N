...
foreach ($rows as $r) {
    ...
    try {
        ...
        if ($psf_dup && $nb_slug) {
            $pdo->prepare("UPDATE monthly_market_stats SET is_active=0 WHERE neighbourhood_slug=? AND data_month=? AND csv_type='hpi_duplex'")
                ->execute([$nb_slug, $month_dt]);
            $pdo->prepare("INSERT INTO monthly_market_stats (neighbourhood_slug,data_month,csv_type,price_per_sqft,sales_count,is_active,created_at) VALUES (?,?,'hpi_duplex',?,?,1,NOW())")
