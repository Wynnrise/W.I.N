...
<div style="flex:1;padding:28px;">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
        <div>
            <h2 style="font-size:18px;font-weight:800;color:#0d0d1a;margin:0;">Developer Accounts</h2>
            <p style="font-size:13px;color:#888;margin:4px 0 0;"><?= count($developers) ?> registered — <?= $pending_devs ?> pending approval</p>
        </div>
    </div>
    ...
    <table class="listings-table">
        ...
        <tr style="<?= $dev['status']==='pending' ? 'background:#fffbeb;' : '' ?>">
            <td style="font-weight:600;">
                <?= htmlspecialchars($dev['full_name']) ?>
                <?php if ($dev['status']==='pending'): ?>
                <span style="display:inline-block;background:#fef3c7;color:#b45309;font-size:10px;font-weight:700;padding:2px 7px;border-radius:20px;margin-left:6px;">NEW</span>
                <?php endif; ?>
            </td>
            ...
            <td>
                ...
                <span style="background:#dcfce7;color:#16a34a;font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;white-space:nowrap;">
                    <i class="fas fa-check me-1"></i>Verified
                </span>
                <?php else: ?>
                <span style="background:#fef3c7;color:#b45309;font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;white-space:nowrap;">
            ...
            <td>
                <?php
                if ($dev['status'] === 'approved')       $sc = 'background:#dcfce7;color:#16a34a;';
                elseif ($dev['status'] === 'suspended')  $sc = 'background:#fee2e2;color:#dc2626;';
                else                                     $sc = 'background:#fef3c7;color:#b45309;';
                ?>
                <span style="<?= $sc ?>font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;text-transform:uppercase;letter-spacing:.5px;">
                    <?= ucfirst($dev['status']) ?>
                </span>
            </td>
            <td style="white-space:nowrap;">
                <?php if ($dev['status'] !== 'approved'): ?>
                <a href="admin.php?dev_approve=<?= $dev['id'] ?>" class="action-btn" style="background:#16a34a;color:#fff;" onclick="return confirm('Approve <?= htmlspecialchars($dev['full_name']) ?>?')">
                    <i class="fas fa-check"></i> Approve
                </a>
                <?php endif; ?>
                <?php if ($dev['status'] !== 'suspended'): ?>
                <a href="admin.php?dev_suspend=<?= $dev['id'] ?>" class="action-btn" style="background:#dc2626;color:#fff;" onclick="return confirm('Suspend this developer?')">
                    <i class="fas fa-ban"></i>
                </a>
                <?php endif; ?>
                <?php if ($dev['status'] !== 'pending'): ?>
                <a href="admin.php?dev_pending=<?= $dev['id'] ?>" class="action-btn" onclick="return confirm('Reset to pending?')">
                    <i class="fas fa-rotate-left"></i>
                </a>
                <?php endif; ?>
                <a href="admin.php?dev_delete=<?= $dev['id'] ?>" class="action-btn" style="background:#ef4444;color:#fff;" onclick="return confirm('Permanently delete this developer account?')">
